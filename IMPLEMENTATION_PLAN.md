...
user-сообщением** через distillctl-seam (`session send`, `if_running=queue`,
...
в шелл сессии, как это делал goose, поэтому вызовы distillctl из агентов сейчас
...
(`distillctl/commands/runtime/spawnGate.ts`). Анонимный вызов — оператор,
...
regex-fallback плана, второй session manager ради `distillctl --parent`,
