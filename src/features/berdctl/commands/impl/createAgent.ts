...
const createAgentSchema = z
  .object({
    ...
    model: z
      ...
      .optional()
      .describe(
        "Id of the model the agent should use (from `berdctl info models`). " +
    ...
    provider: z
      ...
      .optional()
      .describe(
        "Agent harness that serves the model (from `berdctl info harnesses`, " +
    ...
    fast_mode: z
      ...
      .optional()
      .describe(
        "Run the agent's model in fast mode. Only for a model `berdctl info " +
  ...
  .refine(
    ...
    {
      message:
        "effort and fast_mode are checked against the model they run on; pass model (from `berdctl info models`) with them",
      path: ["model"],
    },
  );
...
export const createAgentCommand = defineCommand({
  ...
  description:
    ...
    "mode is checked against what `berdctl info models` reports for the chosen model.",
  helpFooter: `Examples:
  berdctl agent create --name "Reviewer" \\
    --system-prompt "You review diffs for correctness; be terse."
  berdctl agent create --name "Planner" --system-prompt "You plan migrations." \\
...
Result:
  ...
  \`berdctl session create\`. Its chats start on the saved model, effort
