import {
  ...
  setDistillctlTimeouts,
  startDistillctlServer,
  stopDistillctlServer,
  submitDistillctlResult,
  type BridgeRequest,
  type BridgeResult,
...
import { commandBridgeTimeoutMs } from "@/features/distillctl/commands/timeouts";
import { toCommandError } from "@/features/distillctl/commands/types";

/**
 * Module-scoped lifecycle singleton for the distillctl broker.
 *
 ...
 */
export function setDistillctlDesired(next: boolean): void {
  if (next) {
...
async function doStart(): Promise<void> {
  ...
  let started = false;
  try {
    await startDistillctlServer();
    started = true;
    await setDistillctlTimeouts(actionTimeoutsMs());
    state.running = true;
  } catch (error) {
    if (started) {
      try {
        await stopDistillctlServer();
      } catch (stopError) {
        console.error(
          "[distillctl] failed to stop broker after startup failure",
          stopError,
        );
      }
    }
    if (isPluginUnavailableError(error)) {
      ...
      console.warn(
        "[distillctl] plugin unavailable in this build; bridge is inert",
      );
    } else {
      console.error("[distillctl] failed to start broker", error);
    }
  }
}
...
async function doStop(): Promise<void> {
  ...
  try {
    ...
    console.error("[distillctl] failed to stop broker", error);
  }
}
 ...
 */
export async function handleDistillctlRequest(
  ...
  try {
    await submitDistillctlResult(result);
  } catch (error) {
    console.error("[distillctl] failed to submit bridge result", error);
  }
}

/** Test-only: drop all lifecycle state without invoking the plugin. */
export function __resetDistillctlLifecycleForTests(): void {
  state = freshState();
  desired = false;
  applied = false;
  reconcilePromise = null;
