...
function drainQueuedMessage(queuedSessionId: string, ownerId: string): void {
  ...
  if (
    ...
    !isDistillctlCrossSessionQueuedMessage(queuedMessage) ||
  ...
  if (
    deliveryId &&
    hasAcceptedDistillctlDeliveryInTranscript(queuedSessionId, deliveryId)
  ) {
...
export function useDistillctlQueuedMessageDrain(
  queuedSessionId?: string,
  ownerReady = true,
): void {
  useEffect(() => {
