...
export const createSessionCommand = defineCommand({
  ...
  description:
    ...
    "what `distillctl info models` reports for the chosen model. " +
  ...
  execute: async (args, ctx): Promise<CreateSessionResult> => {
    ...
    if (
      ...
      throw new CommandError(
        "invalid_args",
        "--effort and --fast-mode are checked against the model they run on; pass --model-id (from `distillctl info models`) with them.",
      );
    }
    const [
      { enforceDistillctlSpawnAcl, registerDistillctlChildNode },
      { acceptFirstSend },
      { useChatSessionStore },
      { resolveSessionCwd },
      {
