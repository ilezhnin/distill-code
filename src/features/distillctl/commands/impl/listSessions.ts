...
export const listSessionsCommand = defineCommand({
  ...
  helpFooter: `Example:
  distillctl session list --query "review" --limit 10 --json
  ...
  execute: async (args): Promise<ListSessionsResult> => {
    const [
      ...
      { findProjectOrThrow },
      {
        ...
        loadRecentSessionsForDistillctl,
        sessionMetadata,
      },
    ] = await Promise.all([
    ...
    // (and distill-monitor's delivery loop) poll this command, and every page
    // costs an IPC round-trip plus a session-store write the sidebar renders.
    const hostSessions =
      args.project_id || args.query
        ? await loadAllSessionsForDistillctl()
        : await loadRecentSessionsForDistillctl(args.limit);
