...
const archiveSessionSchema = z
  .object({
    ...
    // Kept on the wire for CLI compatibility only. distillctl can no longer
    ...
    discard_changes: z
      ...
      .optional()
      .describe(
        "No effect; kept for compatibility. distillctl never discards local files or changes: when Git cleanup would, the command refuses and the user must confirm in the app.",
      ),
  })
  .strict();
...
and distillctl cannot override that: only the user can confirm the loss, in the app.
It also refuses while the chat still has running terminals: archiving in the app
stops that chat's shells, and distillctl will not end a dev server, a build or a
...
Example:
  distillctl session archive --session-id <session-id>
...
Result:
  ...
  execute: async (args, ctx) => {
    const [{ getAppNavigationController }, { loadSessionForDistillctl }] =
      await Promise.all([
        import("../../navigation"),
        import("../runtime/sessions"),
      ]);
    await loadSessionForDistillctl(args.session_id);
    // Always "reject": distillctl has no way to obtain the user's consent to
...
function archiveFailureMessage(
  ...
  switch (reason) {
    ...
    case "cleanup_requires_discard":
      return `Refusing to archive session "${sessionId}" because Git cleanup would discard local files or changes; distillctl cannot discard them. Ask the user to archive the chat in the app, where they can confirm the loss.`;
