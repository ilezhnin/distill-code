...
export const sendSessionCommand = defineCommand({
  ...
  distillctl session send --session-id <session-id> \\
    ...
    --from "the Distill session handling CI" --delivery-id <stable-id> --json
  ...
  execute: async (args): Promise<SendSessionResult> => {
    const [
      ...
      { findProjectOrThrow },
      {
        ...
        hasAcceptedDistillctlDelivery,
        reserveDistillctlDelivery,
    ...
    const releaseDeliveryReservation = args.delivery_id
      ? reserveDistillctlDelivery(args.session_id, args.delivery_id)
      : undefined;
        ...
        await sendPromptToExistingSessionInBackground(
          ...
          },
          {
            ...
            validateHydratedTranscript: () => {
              if (
                args.delivery_id &&
                hasAcceptedDistillctlDelivery(args.session_id, args.delivery_id)
              ) {
                throw new DistillctlDeliveryAlreadyAcceptedError();
              }
            },
          },
        );
