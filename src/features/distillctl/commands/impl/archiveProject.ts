...
import { DISTILLCTL_BOUNDS, backendArchiveFailedMessage } from "../helpers";
...
Example:
  distillctl project archive --project-id <project-id>
...
Result:
  ...
  execute: async (args) => {
    const [
      { archiveProject },
      { findProjectOrThrow, loadProjectsForDistillctl },
    ] = await Promise.all([
