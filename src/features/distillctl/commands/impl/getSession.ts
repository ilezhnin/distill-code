...
import { DISTILLCTL_BOUNDS, truncate } from "../helpers";
...
Result:
  ...
  execute: async (args): Promise<GetSessionResult> => {
    const [
      { lastSessionMessages },
      { loadSessionForDistillctl, requireSession, sessionMetadata },
    ] = await Promise.all([
      import("@/shared/api/sessionSearch"),
      import("../runtime/sessions"),
    ]);
    const hostSession = await loadSessionForDistillctl(args.session_id);
