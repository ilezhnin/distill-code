...
Examples:
  distillctl project set-startup-mode --project-id <project-id> --mode auto-worktree
  distillctl project set-startup-mode --project-id <project-id> --mode none
...
Result:
  ...
  execute: async (args, ctx): Promise<SetProjectStartupModeResult> => {
    ...
    if (!project) {
      throw new CommandError(
        "project_not_found",
        `No project "${args.project_id}"; list projects with \`distillctl project list\`.`,
      );
    }
    ...
    // Deliberately no distill_project Edit Completed telemetry: this rewrite of
    // projectWorkspaces/workingDirs/useWorktrees is a genuine configuration
    // edit by the event's own params, but distillctl mutations are
    // agent/automation-driven and the event tracks human-driven UI surfaces
    // only — matching the documented distillctl exclusions in the chat send path
    // (fireChatSendTelemetry in useChatSessionController) and `distillctl agent
