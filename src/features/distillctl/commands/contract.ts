...
/**
 * Builders for the distillctl contract artifacts
 * (src-tauri/crates/distillctl/api-surface.json and cli-surface.json).
 *
...
type VerbSpec = {
  ...
  action: string;
  /** One-line summary in `distillctl <noun> --help`'s verb list. */
  about: string;
...
const API_COMMENT =
  ...
  "...fields}} against the loopback endpoint in the distillctl discovery file. " +
  "protocolVersion mirrors PROTOCOL_VERSION in both discovery.rs copies " +
  "(distillctl and plugin crate tests pin them equal). Per action: " +
  "description, fields " +
...
function describeField(
    ...
    if (!values.every((value): value is string => typeof value === "string")) {
      throw new Error(
        `${group}.${action}.${name}: numeric z.enum values are not supported; ` +
          "use string enum values so distillctl can expose them as CLI possible values",
      );
    }
    return { ...base, values };
  }
  ...
  throw new Error(
    `${group}.${action}.${name}: unsupported zod type ` +
      ...
      "the distillctl crate's tree.rs/wire.rs to build a flag for it), then " +
      "regenerate with `pnpm generate:distillctl-contract`",
  );
}
