...
export class DistillctlDeliveryAlreadyAcceptedError extends Error {
  constructor() {
    super("The Distill delivery was already accepted.");
    this.name = "DistillctlDeliveryAlreadyAcceptedError";
  }
}

export function distillctlCrossSessionSendOptions(
  options: { senderLabel?: string; deliveryId?: string } = {},
): ChatSendOptions {
  const senderMetadata = options.senderLabel
    ? { distillSenderLabel: options.senderLabel }
    : {};
  const deliveryMetadata = options.deliveryId
    ? { distillDeliveryId: options.deliveryId }
    : {};
  return {
    userMessageMetadata: {
      origin: DISTILLCTL_CROSS_SESSION_ORIGIN,
      ...senderMetadata,
      ...deliveryMetadata,
    },
    acpPromptMetadata: {
...
export function hasAcceptedDistillctlDelivery(
  sessionId: string,
  deliveryId: string,
): boolean {
  const chatStore = useChatStore.getState();
  return (
    hasAcceptedDistillctlDeliveryInTranscript(sessionId, deliveryId) ||
    (chatStore.queuedMessageBySession[sessionId] ?? []).some(
      (record) =>
        record.payload.sendOptions?.userMessageMetadata?.distillDeliveryId ===
        deliveryId,
    )
  );
}

export function hasAcceptedDistillctlDeliveryInTranscript(
  sessionId: string,
  deliveryId: string,
): boolean {
  return (useChatStore.getState().messagesBySession[sessionId] ?? []).some(
    (message) => message.metadata?.distillDeliveryId === deliveryId,
  );
}

export function reserveDistillctlDelivery(
  sessionId: string,
  deliveryId: string,
): (() => void) | null {
  ...
  if (
    ...
    hasAcceptedDistillctlDelivery(sessionId, deliveryId)
  ) {
    return null;
  }
  ...
  const settlement = (async () => {
    try {
      ...
      await sendPromptInBackground(
        ...
        persona,
        {
          ...(options.sendOptions ?? distillctlCrossSessionSendOptions()),
