/**
 * The spawn ACL for the distillctl path (P42).
 *
 * `distillctl session create` / `session fork` create sessions programmatically.
...
/**
 * Records an anonymous spawn in the app log (`distill.log`, the same channel
 ...
 * built-in host is every distillctl spawn.
 */
export function logAnonymousDistillctlSpawn(args: {
  verb: "create" | "fork";
  ...
  void logRendererEvent(
    "info",
    `[distillctl] session ${args.verb} carried no actor (the agent host does not export AGENT_SESSION_ID); treating the call as the operator and allowing a ${target} session without a spawn ACL check`,
  );
}
...
/**
 * Enforces the spawn ACL for one distillctl-created session. Resolves the
 ...
 */
export function enforceDistillctlSpawnAcl(args: {
  actor: string | null | undefined;
...
/**
 * Registers a distillctl-created session under its actor in the conductor
 ...
 */
export function registerDistillctlChildNode(args: {
  actor: string | null | undefined;
  sessionId: string;
  role: RoleLayer;
  harnessId: string;
