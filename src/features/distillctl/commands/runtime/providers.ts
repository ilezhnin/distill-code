...
import { getDistillctlQueryClient } from "../../bridge/runtimeContext";
...
function sharedDoctorReport(): Promise<DoctorReport | null> {
  const queryClient = getDistillctlQueryClient();
  ...
  if (match.readiness !== "ready") {
    throw new CommandError(
      ...
      (match.readiness === "not_installed"
        ...
        ' The user must fix it in the app; pick a "ready" harness from `distillctl info harnesses`.',
    );
  }
  return match;
}
...
export function resolveRequestedModelSelection(
  ...
  if (!row) {
    ...
    if (listed && !row) {
      ...
      throw new CommandError(
        "model_not_found",
        `Model ${named} is not available on "${harnessId}"; list models with \`distillctl info models\`.`,
      );
    }
  }
  ...
  if (fastMode === true && row?.supports_fast === false) {
    throw new CommandError(
      "fast_not_supported",
      `Model "${baseId}" on "${harnessId}" has no fast mode; omit --fast-mode. Models that have one report "supports_fast": true in \`distillctl info models\`.`,
    );
  }
