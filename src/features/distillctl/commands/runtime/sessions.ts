...
export async function loadAllSessionsForDistillctl(): Promise<HostSessionInfoById> {
  return loadSessionsForDistillctl(null);
}
 ...
 * table.
 */
export async function loadRecentSessionsForDistillctl(
  rowLimit: number,
): Promise<HostSessionInfoById> {
  return loadSessionsForDistillctl(rowLimit);
}

async function loadSessionsForDistillctl(
  ...
  try {
    ...
    // write per page made every `distillctl session list` cost one full sidebar
...
export async function loadSessionForDistillctl(
  sessionId: string,
...
/**
 * Refuses a distillctl mutation that would hide a chat still running shells.
 *
 ...
 * restores nothing, so only the operator may cause it. distillctl is declared
...
export async function refuseChatWithLiveTerminals(
  ...
  throw new CommandError(
    ...
    `Refusing to ${verb} session "${sessionId}" because it still has running terminals; archiving hides the chat and distillctl will not stop the shells. Ask the user to close the terminals, or to archive the chat in the app.`,
  );
}
...
export function sessionMetadata(
  ...
  return {
    ...
    // Kept in the distillctl result shape; sessions only ever live in the main
