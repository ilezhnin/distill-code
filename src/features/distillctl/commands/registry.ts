...
/**
 * The distillctl command surface: one group per domain, dispatching on an
 ...
 * test-only deadline fallback below). The distillctl CLI's noun/verb surface
 * mirrors these groups/actions via src-tauri/crates/distillctl/cli-surface.json,
 * which is generated from this registry's `cli` metadata
 * (`pnpm generate:distillctl-contract`) and asserted fresh on both sides.
 *
 * `cli.noun` is the CLI's (singular) noun for the group; `cli.about` is the
 * noun's one-line entry in `distillctl --help`'s command list; verbs default to
