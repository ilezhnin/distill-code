...
describe("useDistillctlQueuedMessageDrain", () => {
  ...
  it("drains a Distillctl exact head once when its session gains a target", async () => {
    useChatSessionStore.setState({
      sessions: [
        {
          id: "session-1",
  ...
  it("drains a Distillctl exact head once after a source-less targetless state gains an ACP target", async () => {
    useChatSessionStore.setState({
      sessions: [
        {
          id: "session-1",
  ...
  it("ignores unrelated session updates while a Distillctl head stays targetless", () => {
    useChatSessionStore.setState({
      sessions: [
        {
          id: "session-1",
  ...
  it("drains a Distillctl record after editing finishes while idle", async () => {
    const chatStore = useChatStore.getState();
    chatStore.enqueueTransportReadyMessage("session-1", {
      persona: { kind: "inherit" },
  ...
  it("drains distillctl-origin queued messages when the target session becomes idle", async () => {
    const chatStore = useChatStore.getState();
  ...
  it("retries a replacement after the stale Distillctl attempt loses ownership", async () => {
