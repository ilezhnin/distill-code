...
import apiSurface from "../../../../src-tauri/crates/distillctl/api-surface.json";
 ...
 * introspection this test calls (contract.ts). The distillctl crate builds its
 ...
 * holds the same property byte-exactly with `just distillctl-contract-check`.
 */
describe("distillctl API surface contract", () => {
  it("checked-in api-surface.json is fresh (run `pnpm generate:distillctl-contract`)", () => {
    expect(
      JSON.parse(JSON.stringify(buildApiSurfaceContract())),
      "src-tauri/crates/distillctl/api-surface.json is stale; run " +
