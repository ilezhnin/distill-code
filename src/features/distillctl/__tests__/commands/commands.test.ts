...
describe("sessions.send", () => {
  ...
  it("queues behind a composer message that arrives before commit", async () => {
    ...
    ).toEqual(["composer head", "distillctl tail"]);
    expect(
      useChatStore.getState().messagesBySession["session-1"],
    ).toBeUndefined();
  });
  ...
  it("preserves a visible sender label on queued prompts", async () => {
    ...
    const result = await dispatchCommand(
      "sessions",
      {
        ...
        if_running: "queue",
        from: "the Distill session handling distill-monitor implementation",
    ...
    expect(
      ...
      userMessageMetadata: {
        origin: "distillctl_cross_session",
        distillSenderLabel:
          "the Distill session handling distill-monitor implementation",
...
describe("projects", () => {
  ...
  it("archive maps a backend failure to backend_archive_failed", async () => {
    ...
    expect(error.message).toContain("distillctl project list");
  });
