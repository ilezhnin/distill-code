...
- "class" is optional: one of ${modelPreferenceClassIds().join(", ")}. It says how hard THIS step's work is, not what the worker is good at — the same role can be a one-line rename and a week of refactoring — and the operator's own map decides which models each class prefers. Name it when the step is clearly heavier or lighter than its role's usual work; omit it and the role decides as before. It is not a way to ask for a specific model: that is "model".
- "effort" is optional: how hard the step's model reasons, in the model's own words — commonly "low", "medium", "high" or "xhigh". It is its own field and never part of "model": write "model":"gpt-5.6-sol","effort":"xhigh", never "model":"gpt-5.6-sol[xhigh]". Omit it and the step runs at its role's ranked effort or the model's default. A plan naming an effort the step's model does not offer is refused whole, with the efforts it does offer. Name one only when the step clearly needs more or less reasoning than its role usually gets.
- "fast" is optional: true runs the step in its model's fast mode, false keeps fast mode off. A plan asking for fast mode on a model without one is refused whole. Leave it out unless speed matters more than cost for this step.
