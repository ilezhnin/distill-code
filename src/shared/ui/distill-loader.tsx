...
export function DistillLoaderMark({
  ...
  color,
  decorative = false,
  durationMs = DISTILL_LOADER_LOOP_MS,
  height,
  size = 70,
  style,
  width,
  ...
  const markStyle: CSSProperties & Record<`--${string}`, string> = {
    ...
    backgroundSize: `${resolvedSize * DISTILL_LOADER_FRAME_COUNT}px ${resolvedSize}px`,
    "--distill-loader-size": `${resolvedSize}px`,
    ...style,
  };

  if (animated) {
    markStyle.animation = `distill-loader-frames ${durationMs}ms steps(${DISTILL_LOADER_FRAME_COUNT}) infinite`;
  }
...
export type DistillLoaderProps = DistillLoaderMarkProps;

function DistillLoader({
  durationMs = DISTILL_LOADER_LOOP_MS,
  ...props
}: DistillLoaderProps) {
  return (
    <DistillLoaderMark
      {...props}
      data-slot="distill-loader"
      durationMs={durationMs}
    />
  );
}

export { DistillLoader };
