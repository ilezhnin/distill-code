...
// Keys are g2 `namespace__tool` identifiers; values are the human-readable
...
// App-local labels for tools g2 does not define. Add them here rather than in
// `toolLabels.json` so that file stays a faithful mirror of g2 — otherwise a
// re-sync from g2 would silently drop the entry.
const localLabels: Record<string, string> = {};

/**
 * Resolve a g2 `namespace__tool` identifier to its human-readable label.
