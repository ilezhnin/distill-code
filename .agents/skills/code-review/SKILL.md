...
- Use shadcn token names first for shared anatomy (`background`, `foreground`, `card`, `popover`, `muted`, `accent`, `primary`, `destructive`, `border`, `input`, `ring`). Distill extension tokens are allowed only for narrow product-specific surfaces or identities that do not map honestly to shadcn.
