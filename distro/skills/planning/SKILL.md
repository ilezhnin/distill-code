...
- Task-owned paths: <allowlist>
- Delivery boundary: <local handoff | local task-branch commit | authorized remote actions>
...
* [ ] Baseline, task-owned paths, and delivery boundary are recorded.
...
* [ ] Delivery stopped at the exact boundary recorded in the plan.
```
...
Finish with a short summary that names the two artifact paths, states whether user review is required, and identifies the recommended next skill: usually `crossworking` for execution, `asset-pipeline` for asset sourcing/generation/import, or `grill` for stress-testing the plan.
