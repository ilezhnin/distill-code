...
If there is no task diff, use `arch-audit` for a refactor backlog or `unity-codebase-audit` for a read-only Unity risk report. If behavior is broken, use `unity-debug` before simplifying.
...
2. **Select only proven candidates**
   ...
   Named smells to hunt in every pass (frequent agent-generated bloat):
   - Runtime-built hierarchies, UI, or components that a prefab/scene authoring edit replaces.
   - Code-resolved references (`Find`, tag/name lookup, `GetComponentInChildren` chains,
     `Resources.Load`) that one serialized field wired in the scene/prefab replaces.
   - Interfaces, factories, events, or registries with exactly one implementation/subscriber.
   - Fallback and catch-and-continue paths: forbidden by standing project order. Deleting one
     changes error-path behavior by design - report it as a sanctioned fail-loud fix, not as a
     behavior-preserving edit.
   - Speculative configurability: parameters, flags, and settings entries that no call site ever
     varies.
   - Comment and documentation bloat: comments restating code, narration of the change itself,
     unrequested generated docs, and watery sections in task-touched living docs - delete them.
   - Logging on happy paths and per-frame log spam.
...
3. **Apply one narrow pass**
   ...
   - Preserve serialized fields, asset GUIDs, save/network schemas, deterministic ordering, lifecycle behavior, error semantics, authored content, and externally shipped public APIs; internal APIs in this pre-release project may be simplified or deleted when call sites are migrated in the same change.
...
4. **Check the complexity budget**
   ...
   - Documentation counts too: task-touched docs and comments must end shorter or denser, never
     more watery.
   - Do not delete tests merely to shrink the diff; the project lean-test policy still applies — when touching fixtures, prune micro-pins/per-parameter sweeps and keep determinism/rollback, fail-loud contracts, content integrity, and real-bug regressions.
...
5. **Recheck the changed tree**
   ...
   - Record exact commands/results from the recheck; later task-content edits invalidate this focused evidence.
...
6. **Hand off downstream gates**
   - Return the final diff scope, retained-risk notes, and focused-check evidence to the parent.
...
- Worker (`unity-worker` for this project): performs the single simplification pass and reruns the focused baseline check.
- Downstream validator (`unity-test-runner`) and reviewer (`unity-reviewer`): run the initial gate set after this skill returns; this skill does not spawn duplicate gates, while accepted later edits trigger the caller's normal rerun.
...
- Focused-check commands/results from the recheck.
