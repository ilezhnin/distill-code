...
- One coherent validated change per commit: it is self-contained, includes its tests, and the project compiles after it.
- Mimic the user's commit style: read recent `git log` before the first commit and match its shape (in this repository: single-line `type: lowercase imperative summary`, body only for genuinely multi-part changes).
- Keep task-scoped, behavior-preserving simplification in the same commit as the behavior it supports.
- Unity: keep scene/prefab/asset churn in the commit that caused it, and mention it in the message when significant.
- Commits mark completed coherent changes, never phases ("Phase 1", "WIP", checkpoint marks). All commits stay on the agent's own local task branch; squash only on the user's direct request; never push.
