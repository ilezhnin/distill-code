...
5. **Guard the win**
   - Default: record the measured budget where the project keeps such docs, so the win cannot silently regress. Author a PlayMode budget-assertion test (ProfilerRecorder snippet in references) only for a regression that has recurred (lean-test policy).
...
- Guard: budget documented, or a test added for a recurring regression.
