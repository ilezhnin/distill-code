...
7. After editing scripts or assets with filesystem tools (outside the editor), request an asset database refresh through MCP (never during Play Mode - see Guardrails): an unfocused editor does not auto-refresh, and without it the compilation you are waiting for never starts.
...
- Never trigger AssetDatabase refresh or script compilation while Play Mode is active - a mid-play domain reload hard-freezes the editor; stop Play Mode first.
