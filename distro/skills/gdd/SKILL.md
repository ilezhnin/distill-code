...
2. Extract the pillars from the request: the one-sentence fantasy, genre, platform, session shape. Research what the design depends on - delegate external facts (genre references, platform constraints, comparable mechanics) to the read-only `researcher` role instead of guessing. Record every assumption this forces - dimension, perspective, controls, scope; assumptions are grill material for step 9, not settled decisions.
...
9. Grill the draft with the user. For a new game or a multi-milestone MVP, run the full `grill` - always, even when the design feels obvious: even a one-line idea hides decisions the user has not actually made. Derive the questions from the context and the assumptions recorded in step 2; confirm the pillars, the core loop, the MVP cut, and the milestone slicing. For a single scoped feature, record the assumptions in the GDD and confirm them with the user in one batched exchange instead of a full grill.
...
- The selected GDD is a living working contract: update its scoped file in place when the user approves a changed decision; move discarded scope to Later / Cut. Approval binds to the GDD file plus the user's explicit confirmation of material changes; typo-level edits do not invalidate approval - no content-hash bookkeeping.
- An ungrilled GDD is a draft, not a contract: the step 9 grill (full or batched, per scope) is a gate, not a suggestion, no matter how obvious the design looks.
