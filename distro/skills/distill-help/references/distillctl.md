...
`distillctl` is the CLI agents use to drive the visible app — it only exposes
verbs that map to something the user can also see happen in the UI. It is
organized as `distillctl <noun> <verb>` (for example `distillctl session list`,
`distillctl project create --name demo`).

- Discover what exists by asking the tool, not by reciting a memorized list:
  run `distillctl --help` for the noun tree, then `distillctl <noun> --help` for
  its verbs.
- Treat `distillctl <noun> <verb> --help` as the authoritative reference for
...
- Prefer `distillctl` over describing a click path when the user wants
...
- Every `distillctl` verb corresponds to something visible in the app — if a
...
- `distillctl info models` lists every model's `efforts` and `supports_fast`.
...
- `distillctl session create` takes the effort and fast mode as their own flags
  ...
  `fast_not_supported`.
- `distillctl agent create` takes `--effort` and `--fast-mode` next to `--model`,
  checks them the same way, and saves them on the agent, so its chats start on
  that model, effort and fast mode.
- `distillctl session get` and `distillctl session list` report the `effort` and
