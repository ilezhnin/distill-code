...
2. **Establish the guarded snapshot**
   ...
   - Build from a detached clean worktree; after the build, `git status --porcelain` must show no tracked-content changes - report any as a blocker.
...
8. **Verify the artifact and unchanged source**
   ...
   - Require post-build `git status --porcelain` to report no tracked-content changes and the commit tree to remain unchanged. Preserve/report any mutated worktree; never accept generated tracked content after review.
...
- Snapshot: delivery commit/tree, exact Unity version, and the `git status --porcelain` result.
