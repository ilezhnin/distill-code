...
For game-pipeline Prepare delivery, run these commands only in a separate clean detached worktree at the reviewed local commit. Before using a template, load `Invoke-GuardedUnity` from `unity-validate`'s `references/validation-commands.md` (its `finally` postflight reports any source mutation via `git status`). A missing build entry point returns the task to Execute; do not create or edit one during Prepare delivery.
    ...
    Invoke-GuardedUnity {
        try {
            ...
            if ($usesAddressables) {
                ...
                [System.IO.File]::WriteAllLines((Join-Path $projectRoot "Logs/addressables-generated-paths.txt"), $generated, (New-Object System.Text.UTF8Encoding($false)))
            }
        }
    }
}
