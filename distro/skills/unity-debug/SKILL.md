...
- For diagnosis, analysis, or root-cause-only requests, run capture, reproduction, localization, and minimization, then report the cause and proposed regression guard without editing production files.
- Continue into fixing and regression changes only when the user asks to fix, resolve, or implement the correction.
...
7. **Verify**
   ...
   - Then run the cheapest sufficient broader check via unity-validate.
