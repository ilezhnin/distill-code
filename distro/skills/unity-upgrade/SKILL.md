...
2. **Stage the editor upgrade**
   ...
   - Gate each stage on the validation ladder below before starting the next; commit per green stage. Per-stage commits live on the upgrade's own local task branch in the user's commit style and stay local; the user integrates the branch.
