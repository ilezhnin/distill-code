//! distillctl broker: the in-app command broker for the distillctl CLI.
//!
...
mod plugin {
    ...
    impl fmt::Display for AppCommandDispatchError {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            match self {
                Self::PluginNotInitialized => write!(f, "distillctl plugin is not initialized"),
                Self::BridgeUnavailable => write!(f, "app window unavailable"),
                Self::RendererDropped => write!(f, "renderer dropped distillctl command"),
                Self::Timeout => write!(f, "renderer timed out handling distillctl command"),
                Self::Command { code, message } => {
                    write!(f, "distillctl command failed ({code}): {message}")
                }
            }
        }
    }
    ...
    /// the distillctl CLI, without going through the loopback HTTP broker.
    pub async fn dispatch_app_command<R: Runtime>(
        ...
        let Some(state) = app.try_state::<DistillctlState>() else {
    ...
    async fn start<R: Runtime>(
        ...
        let handle = server::start_server(ctx)
            .await
            .map_err(|err| format!("failed to start distillctl server: {err}"))?;
        ...
        if let Err(err) = discovery::write_discovery_file(&path, port, pid, generation) {
            handle.shutdown();
            return Err(format!(
                "failed to write distillctl discovery file {}: {err}",
                path.display()
            ));
        }
        ...
        *server_slot = Some(handle);
        log::info!("[distillctl] listening on 127.0.0.1:{port} (generation {generation})");
    ...
    #[tauri::command]
    async fn status(state: State<'_, DistillctlState>) -> Result<BrokerStatus, String> {
    ...
    #[tauri::command]
    async fn stop(state: State<'_, DistillctlState>) -> Result<(), String> {
        let mut server_slot = state.server.lock().await;
        if let Some(handle) = server_slot.take() {
            let port = handle.port;
            handle.shutdown();
            state.remove_discovery_file();
            log::info!("[distillctl] stopped server on 127.0.0.1:{port}");
        }
        Ok(())
    }
    ...
    #[tauri::command]
    fn set_timeouts(state: State<'_, DistillctlState>, timeouts: HashMap<String, u64>) {
        let count = timeouts.len();
        state.timeouts.set(timeouts);
        log::info!("[distillctl] timeouts updated ({count} commands)");
    }

    #[tauri::command]
    fn submit_result(state: State<'_, DistillctlState>, result: BridgeResult) {
        state.bridge.resolve(result);
    }

    pub fn init<R: Runtime>() -> TauriPlugin<R> {
        tauri::plugin::Builder::new("distillctl")
            ...
            .setup(|app, _api| {
                ...
                app.manage(DistillctlState::new());
                Ok(())
            })
            .on_event(|app, event| {
                if let RunEvent::Exit = event {
                    if let Some(state) = app.try_state::<DistillctlState>() {
                        state.remove_discovery_file();
                    }
                }
            })
