//! Per-instance discovery ("lock") file the distillctl CLI reads to find the
...
/// file and echoed by `GET /v1/ping`. The distillctl CLI carries its own copy
/// (src-tauri/crates/distillctl); the CLI does not depend on this crate —
/// bump both together.
#[cfg_attr(not(feature = "server"), allow(dead_code))]
pub const PROTOCOL_VERSION: u32 = 6;

/// Directory under the app data dir holding the per-instance discovery files.
pub const DISCOVERY_DIR_NAME: &str = "distillctl";
...
/// `<app_data_dir>/distillctl/control-<pid>.json`. Per-instance (pid
...
pub(crate) fn remove_discovery_file(path: &Path) {
    if let Err(err) = std::fs::remove_file(path) {
        if err.kind() != std::io::ErrorKind::NotFound {
            log::warn!(
                "[distillctl] failed to remove discovery file {}: {err}",
                path.display()
            );
        }
    }
...
mod tests {
    ...
    const API_SURFACE: &str = include_str!("../../../crates/distillctl/api-surface.json");
    ...
    fn write_and_remove_lifecycle() {
        let base =
            std::env::temp_dir().join(format!("distillctl-discovery-test-{}", std::process::id()));
