...
mod tests {
    ...
    #[test]
    fn collects_repository_instructions_in_ancestor_order() {
        let repo = tempfile::tempdir().unwrap();
        fs::create_dir_all(repo.path().join(".git")).unwrap();
        let workspace = repo.path().join("pkg");
        fs::create_dir_all(workspace.join(".distill")).unwrap();
        fs::write(repo.path().join("AGENTS.md"), "Repository rules").unwrap();
        fs::write(workspace.join("AGENTS.md"), "Package rules").unwrap();
        fs::write(
            workspace.join(".distill/AGENTS.md"),
            "Custom local document",
        )
        .unwrap();

        let files =
            collect_workspace_instruction_files(vec![workspace.to_string_lossy().into_owned()]);

        assert_eq!(files.len(), 2);
        assert_eq!(files[0].content, "Repository rules");
        assert_eq!(files[1].content, "Package rules");
        assert_eq!(files[0].workspace_paths, files[1].workspace_paths);
    }
}
