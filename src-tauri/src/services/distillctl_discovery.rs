//! Stale distillctl discovery-file cleanup.
//!
//! Each app instance's distillctl broker writes a discovery file at
//! `<app_data_dir>/distillctl/control-<app_pid>.json` and deletes it on
...
//! distillctl feature is off.

use std::path::Path;

use tauri_plugin_distillctl::{owner_pid_from_discovery_file_name, DISCOVERY_DIR_NAME};
...
pub fn sweep_stale_discovery_files(app_data_dir: &Path) {
    ...
    let entries = match std::fs::read_dir(&dir) {
        ...
        Err(error) => {
            log::warn!(
                "Failed to read distillctl discovery dir {}: {error}",
                dir.display()
            );
            return;
        }
    ...
    for entry in entries.flatten() {
        ...
        let path = entry.path();
        log::info!(
            "Removing stale distillctl discovery file {}",
            path.display()
        );
        if let Err(error) = std::fs::remove_file(&path) {
            log::warn!(
                "Failed to remove stale distillctl discovery file {}: {error}",
                path.display()
            );
        }
    }
