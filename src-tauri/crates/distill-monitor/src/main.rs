...
const FOREGROUND_ENV: &str = "DISTILL_MONITOR_FOREGROUND";
const LAUNCH_TOKEN_ENV: &str = "DISTILL_MONITOR_LAUNCH_TOKEN";
...
/// well below it: the distillctl path and the control-lock path are both resolved
...
const PENDING_HEADER_PREFIX: &str = "DISTILL_MONITOR_PENDING_V1 ";
...
#[command(
    ...
    about = "Run a long-lived command and wake its owning Distill session"
)]
struct Cli {
    #[command(subcommand)]
...
enum MonitorCommand {
    ...
    Run {
        ...
        instructions: String,
        /// Distill session that owns the monitor.
...
impl PendingState {
    ...
    fn delivery_id(&self, paths: &StatePaths) -> String {
        ...
        format!("distill-monitor-{state_hash:016x}-{}", self.generation)
    }
...
fn state_base_dir() -> PathBuf {
    if let Some(explicit) = env::var_os("DISTILL_MONITOR_STATE_DIR") {
        return PathBuf::from(explicit);
    }
    #[cfg(target_os = "windows")]
    if let Some(local) = env::var_os("LOCALAPPDATA") {
        return PathBuf::from(local).join("Distill").join("monitor");
    }
    #[cfg(unix)]
    {
        if let Some(runtime) = env::var_os("XDG_RUNTIME_DIR") {
            return PathBuf::from(runtime).join("distill-monitor");
        }
        if let Some(state) = env::var_os("XDG_STATE_HOME") {
            return PathBuf::from(state).join("distill").join("monitor");
        }
    ...
    env::temp_dir().join(format!("distill-monitor-{}", current_user_suffix()))
}
...
fn main() -> ExitCode {
    match run_cli() {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("distill-monitor: {error}");
            ExitCode::FAILURE
        }
    }
}
...
/// The `distillctl` run for each delivery: see [`CREATE_NO_WINDOW`].
#[cfg(windows)]
...
fn flush_pending(
    ...
    let binaries = distillctl_candidates();
...
/// limit fails inside `CreateProcessW` before distillctl ever runs.
...
fn lock_candidates() -> Vec<PathBuf> {
    let Some(explicit) = env::var_os("DISTILLCTL_LOCK").map(PathBuf::from) else {
        return Vec::new();
    };
...
fn distillctl_candidates() -> Vec<OsString> {
    let mut candidates = Vec::new();
    if let Some(explicit) = env::var_os("DISTILLCTL_BIN") {
...
mod tests {
    ...
    fn failed_atomic_replace_preserves_pending_output() {
        let root = env::temp_dir().join(format!(
            "distill-monitor-atomic-write-test-{}",
            std::process::id()
        ));
    ...
    fn delivery_candidates() -> (Vec<PathBuf>, Vec<OsString>) {
        (
            vec![PathBuf::from(
                "C:\\Users\\a-fairly-long-user-name\\AppData\\Local\\Distill\\control-0123456789abcdef.json",
            )],
            vec![OsString::from(
                "C:\\Program Files\\Distill\\resources\\distillctl.exe",
            )],
        )
    }
    ...
    fn fits(chunk: &[u8], instructions: &str) -> bool {
        ...
        build_delivery_prompt("build", chunk, instructions).is_ok_and(|prompt| {
            delivery_fits_command_line(
                ...
                &prompt,
                RunningMode::Queue,
                "distill-monitor-0123456789abcdef-4242-1757000000000000000-7",
            )
        })
    }
    ...
    #[test]
    fn running_mode_matches_distillctl_values() {
