//! distillctl — thin CLI client for the Distill desktop app's control broker.
//!
//! All command semantics (validation, defaults, side effects) live app-side
//! in the distillctl command registry; this binary only maps flags onto
...
//! (`pnpm generate:distillctl-contract`), which also author all help prose
...
/// (`distillctl … --json | head`) is routine CLI usage: a broken pipe counts
...
mod tests {
    ...
    fn contract_validates_cleanly() {
        ...
        assert!(
            ...
            "\nthe CLI contract is inconsistent:\n\n  - {}\n\n\
             ...
             then regenerate with `pnpm generate:distillctl-contract` \
             (see .agents/skills/distillctl-new-command/SKILL.md).\n",
            errors.join("\n  - ")
        );
    }
    ...
    fn actor_global_parses_and_stays_out_of_wire_args() {
        let matches = try_parse(&["distillctl", "session", "list", "--actor", "20260830_7"])
            .expect("actor global parses");
        let globals = wire::globals(&matches);
        assert_eq!(globals.actor.as_deref(), Some("20260830_7"));

        let (_, body) = wire_of(&["distillctl", "session", "list", "--actor", "20260830_7"]);
        assert!(
        ...
        for (noun, spec) in &surface_nouns() {
            ...
            for (verb, verb_spec) in spec.get("verbs").and_then(Value::as_object).unwrap() {
                ...
                let sub = cmd
                    ...
                    .unwrap_or_else(|| panic!("clap has `distillctl {noun} {verb}`"));
                for field in &fields {
                    ...
                    let arg = sub
                        ...
                        .unwrap_or_else(|| {
                            panic!("--{long} is missing on `distillctl {noun} {verb}`")
                        });
                    assert_eq!(
                        arg.is_required_set(),
                        required,
                        "--{long} requiredness on `distillctl {noun} {verb}` diverges from \
        ...
        for (noun, spec) in &surface_nouns() {
            ...
            for (verb, verb_spec) in spec.get("verbs").and_then(Value::as_object).unwrap() {
                let extra = minimal_invocation(noun, verb).unwrap_or_else(|| {
                    panic!("add a minimal invocation for `distillctl {noun} {verb}`")
                });
                let argv: Vec<&str> = [vec!["distillctl", noun, verb], extra].concat();
    ...
    fn optional_flags_are_omitted_from_the_wire() {
        let (_, args) = wire_of(&["distillctl", "session", "create", "--prompt", "hi"]);
        assert_eq!(
            Value::Object(args),
    ...
    fn session_archive_maps_boolean_flags_onto_the_wire() {
        ...
        let (_, args_without_flag) =
            wire_of(&["distillctl", "session", "archive", "--session-id", "s"]);
        assert_eq!(
    ...
    fn numeric_bounds_are_enforced_client_side() {
        for out_of_range in ["0", "101"] {
            assert!(
                try_parse(&["distillctl", "session", "list", "--limit", out_of_range]).is_err(),
    ...
    fn rendered_long_help(path: &[&str]) -> String {
        ...
        let mut bin_name = String::from("distillctl");
        for name in path {
            target = target
        ...
        // `distillctl …` invocation path instead of the bare leaf name.
...
checked against what `distillctl info models` reports for the chosen model. Use
...
Usage: distillctl session create [OPTIONS] --prompt <PROMPT>
...
Options:
      ...
      --harness-id <HARNESS_ID>
          Agent harness to run the session on (from `distillctl info harnesses`,
          e.g. "claude-acp", "codex-acp"). Defaults to the app default.

      --model-id <MODEL_ID>
          Id of the model to use (from `distillctl info models`). An old id with
      ...
      --effort <EFFORT>
          ...
          "high", "xhigh"). Must be one of the efforts `distillctl info models`
          lists for the chosen model, so it requires --model-id. Omit it to run
          at the model's default.

      --fast-mode
          Run the model in fast mode. Only for a model `distillctl info models`
          reports with "supports_fast": true, so it requires --model-id. Omit it
          to keep the model's default.

      --agent-id <AGENT_ID>
          Id of the agent (persona) to use (from `distillctl agent list`).
...
Examples:
  distillctl session create --prompt "Triage the failing nightly build" \
    --harness-id claude-acp --from "the release orchestrator" --json
  distillctl session create --prompt "Plan the migration" \
    --harness-id codex-acp --model-id gpt-5.6-sol --effort xhigh --fast-mode
  distillctl session create --prompt "Implement the fix" \
...
Result:
  ...
  `distillctl session get --session-id <session_id>`.
"#;

    const EXPECTED_SESSION_MOVE_HELP: &str = r#"Move a chat session into a project; the session list in the app regroups
immediately.

Usage: distillctl session move [OPTIONS] --session-id <SESSION_ID> --project-id <PROJECT_ID>
...
Example:
  distillctl session move --session-id <session-id> --project-id <project-id>
...
Usage: distillctl session clear-project [OPTIONS] --session-id <SESSION_ID>
...
"#;
    ...
    /// `cargo test -p distillctl dump_rendered_help -- --ignored`, review the
