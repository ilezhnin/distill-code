//! Discovery-file resolution: how distillctl finds the app's control endpoint.
...
/// `PROTOCOL_VERSION` in the `tauri-plugin-distillctl` crate
/// (src-tauri/plugins/distillctl) — the CLI does not depend on the plugin
...
pub const NOT_UNDER_APP: &str =
    "distillctl must run inside a Distill desktop app session (the app sets this up automatically)";

const REREAD_DELAY: Duration = Duration::from_millis(200);

/// Shape of the discovery file the distillctl broker writes on start
/// (`<app-data>/distillctl/control-<app-pid>.json`). Duplicated by hand from
/// the writer's struct in `tauri-plugin-distillctl`; keep in sync.
...
/// The lock path comes from `--lock-path` or `DISTILLCTL_LOCK` (clap merges
...
pub fn load_with_retry(path: &Path) -> Result<DiscoveryFile, Failure> {
    load(path)
        ...
        .map_err(|err| {
            Failure::env(format!(
                "the Distill desktop app's control endpoint is unavailable ({err}); \
                 the app may not be running or app control may be disabled. \
                 confirm Distill is running, app control is enabled, and this \
                 command is running inside a Distill-started agent session."
            ))
        })
}
