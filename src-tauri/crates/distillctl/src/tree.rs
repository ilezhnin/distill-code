//! Builds the distillctl clap Command tree at startup from the embedded
...
Control the Distill desktop app from the command line.

distillctl talks to the running Distill desktop app and acts on what the user
sees there:
...
Examples:
  ...
  distillctl session list --query \"standup\" --json
  distillctl session get --session-id <session-id> --messages 5 --json

  # Start a background session on a specific agent harness
  distillctl info harnesses --json
  distillctl session create --prompt \"Summarize open code reviews\" \\
    --harness-id claude-acp

  # Send a follow-up into an existing session without opening it
  distillctl session send --session-id <session-id> \\
...
Exit codes:
  ...
  2  transport failure between distillctl and the app — confirm Distill is running,
     app control is enabled, and this is a Distill-started agent session
  3  not running inside a Distill desktop app session, the app is not running,
     or this distillctl no longer matches the app — stderr explains which";

pub fn build_cli(contract: &Contract) -> Command {
    debug_assert!(
        crate::validate::contract_errors(contract).is_empty(),
        "the embedded contract is inconsistent; run `cargo test -p distillctl` \
         for the full error list"
    );

    let mut cmd = Command::new("distillctl")
        .bin_name("distillctl")
        .term_width(HELP_WIDTH)
        .about("Control the Distill desktop app (sessions, projects, agents, skills) from the command line")
            ...
            Arg::new("lock_path")
                ...
                .help(
                    "Path to the app's control discovery file (internal plumbing; \
                     the app sets DISTILLCTL_LOCK in every agent session it spawns)",
                ),
        )
...
fn noun_command(contract: &Contract, noun: &str, spec: &Noun) -> Command {
    let mut cmd = Command::new(noun.to_string())
        ...
        .subcommand_required(true);
    // Hidden plural aliases forgive `distillctl sessions list` etc. without
