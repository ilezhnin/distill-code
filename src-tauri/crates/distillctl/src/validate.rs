...
//! distillctl`) and by a debug assertion in `tree::build_cli`. The artifacts
...
pub fn contract_errors(contract: &Contract) -> Vec<String> {
    ...
    for (noun, spec) in nouns {
        ...
        for (verb, verb_spec) in &spec.verbs {
            ...
            // stub's marker.
            let module = format!("`distillctl {noun} {verb}`'s command module");
            check_prose(
            ...
            let Some(action_spec) = contract.action(&spec.group, action) else {
                errors.push(format!(
                    "api-surface.json is missing `{}.{action}` (for `distillctl {noun} {verb}`)",
                    spec.group
                ));
                continue;
            };
...
mod tests {
            ...
            "session": {
                ...
                "verbs": {
                    "list": {
                        "action": "list",
                        "about": "List sessions",
                        "afterHelp": "Example: distillctl session list"
                    }
                }
            }
        }
    ...
    fn mismatched_protocol_version_is_reported() {
        let api = MINIMAL_API.replace("\"protocolVersion\": 6", "\"protocolVersion\": 999");
        let errors = errors_for(&api, MINIMAL_SURFACE);
