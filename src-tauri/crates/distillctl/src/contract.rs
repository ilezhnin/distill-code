...
//! (`pnpm generate:distillctl-contract`); zod schemas and the command-module
...
pub struct Noun {
    ...
    pub group: String,
    /// One-line entry in `distillctl --help`'s command list.
    pub about: String,
...
pub struct Verb {
    ...
    pub action: String,
    /// One-line entry in `distillctl <noun> --help`'s verb list.
    pub about: String,
...
impl Contract {
    ...
    /// programmer error caught by `cargo test -p distillctl`, never a runtime
